...
class AdminsSubjectList extends StatelessWidget{
   @override
...
class AdminsSubjectListPage extends StatefulWidget{
  AdminsSubjectListPage({Key key, this.title}) : super(key: key);
  final String title;

  @override
  MyAdminsSubjectListPageState createState() => MyAdminsSubjectListPageState();

}
class MyAdminsSubjectListPageState extends State<AdminsSubjectListPage>{
                              ...
                              onTap: (){
                                Navigator.pop(context);
                                  FirebaseDatabase.instance.reference().child("Subject").
                                    ...
                                    Map<dynamic, dynamic> values = snapshot.value;
                                    values.forEach((key,values) {
                                        FirebaseDatabase.instance.reference().child("Subject").child(key).once().then((DataSnapshot dsnapshot){
                                                Map<dynamic, dynamic> vvalues = dsnapshot.value;
                                                vvalues.forEach((kkey,vvalues){
                                                  if(kkey.compareTo(entry)==0){
                                                    // print(entry+" "+kkey);
                                                    FirebaseDatabase.instance.reference().child("Subject").child(key).child(entry).remove();
                                                  }
                                                });
